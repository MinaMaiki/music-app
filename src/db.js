import img1 from "./assists/imgs/الدرس الاول.png";
import img2 from "./assists/imgs/الدرس الاول.jpeg";
import img3 from "./assists/imgs/الدرس الثالث.png";
import img4 from "./assists/imgs/الدرس الرابع.png";
import img5 from "./assists/imgs/الخامس.png";
import img6 from "./assists/imgs/فا دييز.png";
import img7 from "./assists/imgs/مي نص بيمول.png";
import img8 from "./assists/imgs/كرد.png";
import img9 from "./assists/imgs/دوكاه.png";
import img10 from "./assists/imgs/راست.png";
import img11 from "./assists/imgs/لا حسيني.png";
import img12 from "./assists/imgs/سي اوج.png";
import img13 from "./assists/imgs/سي عجم.png";
import img14 from "./assists/imgs/دو كردان.png";
import imgnote from "./assists/imgs/WhatsApp Image 2024-10-28 at 1.50.07 AM.png";
import imgwoman from "./assists/imgs/Woman.png";

export const arr = [
  {
    name: "الدرس الاول : مقدمه تعريفيه عن آلة الناي",
    id: "0",
    img: img1,
    text: "الناي كلمة فارسية ومعناها المزمار وقد أطلق عليها عدة أسماء في بلدان مختلفة ( القصبة -الغابة - القصابة - الشبابة - المنجيزة - الصرناي ) وكلها اسماء لآلة واحدةوهي عباره عن قصبة مجوفة مأخوذة من نبات الغاب مفتوحة من الطرفين تتكون من تسع عقل بها ستة ثقوب أمامية وثقب واحد من الخلف",
    vid: "https://www.youtube.com/embed/uA-h1CmScHQ?autoplay=0",
  },
  {
    name: "الدرس الثاني : المسكه الصحيحه لاله الناي  ",
    id: "1",
    img: imgwoman,
    text: "المسكه الصحيحه لآله الناي من خلال رفع الرأس والوجه إلي الأمام والأصابع فوق الثقوب بإحكام والإلهام في اليد اليسري فوق الثقب الخلفي، وإبهام اليد اليمني من الخلف لتثبيت القصبه بين اليدين ،ووضع الطرف الاعلي علي حافة الشفتين من الخارج ،وهي مضمونتين دون الضغط عليهم ويكون الناي مائلاً قليلاً إلي اليمين .",
    vid: "https://www.youtube.com/embed/uA-h1CmScHQ?autoplay=0",
  },
  {
    name: "الدرس الثالث: كيفية إصدار الصوت من آلة الناي",
    id: "2",
    img: img3,
    text: `لخطوات المتبعة لإصدار الصوت:<br/>
1- إسترخاء الجسم وجميع عضلات الوجه.<br/>
2- مسك آلة الناي مع ثني الذراعين من ناحية الكوع.<br/>
3- وضع أصابع اليد على الثقوب ويكون كف اليد بشكل مقوس.<br/>
4- وضع الآلة على شفاه العازف وخروج الهواء من الفم بشكل مستقيم باستخدام طرف اللسان إلى داخل الآلة.<br/>
5- إغلاق جميع ثقوب الآلة بإحكام حتى لا يتسرب منها الهواء أثناء العزف.`,
    vid: "",
  },

  {
    name: "الدرس الرابع  : النفخه الأولي لاله الناي ",
    id: "3",
    img: imgwoman,
    text: "وهي تنتج بالنفخ الضعيف مع فتح الشفتين قليلاً لتعطي طبقة صوتية غليظة وهي من درجة الراست الي النوا.",
    vid: "",
  },
  {
    name: "ـ كيفيه عزف نغمه صول ( نوا )",
    id: "4",
    img: img4,
    text: "",
    vid: "Video Url",
    note: imgnote,
  },
  {
    name: "ـ كيفيه عزف نغمه فا ( جهركاه )",
    id: "5",
    img: img5,
    text: "",
    vid: "Video Url",
  },
  {
    name: "كيفيه عزف نغمه فا دييز ( حجاز )",
    id: "6",
    img: img6,
    text: "",
    vid: "Video Url",
  },
  {
    name: "ـ كيفيه عزف نغمه مي نصف بيمول ( سيكا )",
    id: "7",
    img: img7,
    text: "",
    vid: "Video Url",
  },
  {
    name: "كيفيه عزف نغمه مي بيمول ( كرد ) ",
    id: "8",
    img: img8,
    text: "",
    vid: "Video Url",
  },
  {
    name: " كيفيه عزف نغمه ري ( دوكاه )",
    id: "9",
    img: img9,
    text: "",
    vid: "Video Url",
  },
  {
    name: " كيفيه عزف نغمه دو ( راست )",
    id: "10",
    img: img10,
    text: "",
    vid: "Video Url",
  },
  {
    name: "الدرس الخامس: النفخه الثانيه لاله الناي ",
    id: "11",
    img: imgwoman,
    text: "وهي تنتج بالنفخ متوسط القوة مع ضم الشفتين قليلاً لتعطي النغمات ما بين القرار والجواب ، وتعتبر طبقة متوسطة الحدة وهي من درجة النوا حتي درجة المحير .",
    vid: "",
  },
  {
    name: " كيفيه عزف نغمه  لا(حسيني )",
    id: "12",
    img: img11,
    text: "",
    vid: "Video Url",
  },
  {
    name: " كيفيه عزف نغمه سي نصف بيمول ( أوج ) ",
    id: "13",
    img: img12,
    text: "",
    vid: "Video Url",
  },
  {
    name: "( كيفيه عزف نغمه سي بيمول ( عجم ",
    id: "14",
    img: img13,
    text: "",
    vid: "Video Url",
  },
  {
    name: "كيفيه عزف نغمه دو جواب ( كردان )",
    id: "15",
    img: img14,
    text: "",
    vid: "Video Url",
  },
  {
    name: "الدرس السادس :مقام الراست",
    id: "16",
    img: "",
    text: "",
    vid: "Video Url",
  },
  {
    name: "الدرس السابع : مقام البياتي",
    id: "17",
    img: "",
    text: "",
    vid: "Video Url",
  },
  {
    name: "الدرس الثامن : مقام الحجاز",
    id: "18",
    img: "",
    text: "",
    vid: "Video Url",
  },
  {
    name: "الدرس التاسع : مقام الصبا",
    id: "19",
    img: "",
    text: "",
    vid: "Video Url",
  },
];
